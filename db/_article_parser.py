...
from db.models import ArticleList, Article
...
def get_article(website_url):
    ...
    try:
        api_count += 1
        print('get_article with ' + website_url)
        ...
        if 'article' in response_list[0]:
            print('article in response_list[0]')
            print(response_list[0]['article'])
            return response_list[0]['article']
        else:
            print('article not in response_list[0]')
            return None
    except Exception as e:
        print(e.strerror)
        return None
